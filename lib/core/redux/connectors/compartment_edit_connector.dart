...
class CompartmentEditorConnector extends StatelessWidget {
  ...
  CompartmentEditorConnector(
      {Key key, @required this.isEditing, this.compartment});
...
class ViewModel extends BaseModel<AppState> {
  ...
  ViewModel fromStore() => ViewModel.build(
      onSaved: (id, name, color, icon) => dispatch(
          SaveCompartmentAction(id: id, name: name, color: color, icon: icon)));
}
